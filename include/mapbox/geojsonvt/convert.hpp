...
struct Converter {
    ...
    Converter(const double tolerance_) : tolerance(tolerance_) {
    }

    vt_point operator()(const mapbox::geometry::point<double>& p) {
    ...
    vt_line_string operator()(const mapbox::geometry::line_string<double>& points) {
    ...
    vt_linear_ring operator()(const mapbox::geometry::linear_ring<double>& ring) {
    ...
    vt_multi_point operator()(const mapbox::geometry::multi_point<double>& points) {
    ...
    vt_multi_line_string operator()(const mapbox::geometry::multi_line_string<double>& lines) {
    ...
    vt_polygon operator()(const mapbox::geometry::polygon<double>& rings) {
        vt_polygon result;
    ...
    vt_multi_polygon operator()(const mapbox::geometry::multi_polygon<double>& polygons) {
...
inline vt_features convert(const mapbox::geometry::feature_collection<double>& features,
    ...
    for (const auto& feature : features) {
        projected.emplace_back(vt_geometry(mapbox::geometry::geometry<double>::visit(
                                   feature.geometry, Converter(tolerance))),
                               feature.properties);
    }
    return projected;
}
